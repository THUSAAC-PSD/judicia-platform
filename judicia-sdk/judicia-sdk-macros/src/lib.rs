...
use syn::{
    parse_macro_input, Fields, Ident, ItemStruct, Lit, Type, punctuated::Punctuated, Token,
};
...
pub fn judicia_plugin(_args: TokenStream, input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as ItemStruct);

    match expand_judicia_plugin(input) {
        Ok(tokens) => tokens.into(),
        Err(err) => err.to_compile_error().into(),
    }
}

fn expand_judicia_plugin(mut input: ItemStruct) -> syn::Result<TokenStream2> {
...
/// ```
#[proc_macro_attribute]
pub fn frontend_component(_args: TokenStream, input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as ItemStruct);
    
    match expand_frontend_component(input) {
        Ok(tokens) => tokens.into(),
        Err(err) => err.to_compile_error().into(),
    }
}

fn expand_frontend_component(input: ItemStruct) -> syn::Result<TokenStream2> {
    ...
    Ok(quote! {
        ...
        impl judicia_sdk_runtime::frontend::FrontendComponent for #component_name {
            fn render(&self, props: &serde_json::Value) -> Result<String, wasm_bindgen::JsValue> {
                self.generate_html(props)
            }
            
            fn handle_event(&mut self, event: &str, data: &serde_json::Value) -> Result<(), wasm_bindgen::JsValue> {
...
impl ToTokens for CapabilityDefinition {
...
fn generate_metadata_init(metadata: &PluginMetadata) -> TokenStream2 {
    ...
    quote! {
        impl judicia_sdk_runtime::PluginMetadata for MyPlugin {
...
fn generate_capability_declarations(capabilities: &[String]) -> TokenStream2 {
    let cap_exprs: Vec<TokenStream2> = capabilities.iter().map(|cap| {
        ...
        quote! { judicia_sdk_runtime::Capability::#cap_ident }
    }).collect();
    
    quote! {
        const REQUIRED_CAPABILITIES: &[judicia_sdk_runtime::Capability] = &[#(#cap_exprs),*];
        
        #[wasm_bindgen::prelude::wasm_bindgen]
        pub fn get_required_capabilities() -> js_sys::Array {
            let array = js_sys::Array::new();
            for cap in REQUIRED_CAPABILITIES {
                array.push(&wasm_bindgen::JsValue::from_str(&cap.to_string()));
            }
            array
        }
    }
...
fn generate_plugin_impl(plugin_name: &Ident, metadata: &PluginMetadata) -> TokenStream2 {
    quote! {
        impl judicia_sdk_runtime::Plugin for #plugin_name {
            fn new() -> Self {
                Self {
            ...
            fn metadata(&self) -> judicia_sdk_runtime::PluginInfo {
                judicia_sdk_runtime::PluginInfo {
...
fn generate_wasm_exports(plugin_name: &Ident) -> TokenStream2 {
    ...
    quote! {
        ...
        #[wasm_bindgen::prelude::wasm_bindgen]
        pub async fn plugin_initialize(context: wasm_bindgen::JsValue) -> Result<(), wasm_bindgen::JsValue> {
            let context: judicia_sdk_runtime::PluginContext = serde_wasm_bindgen::from_value(context)
                .map_err(|e| wasm_bindgen::JsValue::from_str(&format!("Failed to deserialize context: {}", e)))?;
        ...
        #[wasm_bindgen::prelude::wasm_bindgen]
        pub async fn plugin_handle_event(event: wasm_bindgen::JsValue) -> Result<(), wasm_bindgen::JsValue> {
            let event: judicia_sdk_runtime::PlatformEvent = serde_wasm_bindgen::from_value(event)
        ...
        #[wasm_bindgen::prelude::wasm_bindgen]
        pub fn plugin_info() -> wasm_bindgen::JsValue {
            unsafe {
                if let Some(ref plugin) = &PLUGIN_INSTANCE {
                    serde_wasm_bindgen::to_value(&plugin.metadata()).unwrap_or(wasm_bindgen::JsValue::NULL)
                } else {
        ...
        #[wasm_bindgen::prelude::wasm_bindgen]
        pub async fn plugin_cleanup() -> Result<(), wasm_bindgen::JsValue> {
            unsafe {
                if let Some(ref mut plugin) = &mut PLUGIN_INSTANCE {
...
fn generate_component_exports(component_name: &Ident) -> TokenStream2 {
    quote! {
        ...
        impl #component_name {
            ...
            #[wasm_bindgen::prelude::wasm_bindgen]
            pub fn render(&self, props: wasm_bindgen::JsValue) -> Result<String, wasm_bindgen::JsValue> {
            ...
            #[wasm_bindgen::prelude::wasm_bindgen]
            pub fn handle_event(&mut self, event: &str, data: wasm_bindgen::JsValue) -> Result<(), wasm_bindgen::JsValue> {

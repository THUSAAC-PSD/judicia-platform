...
pub mod time {
    ...
    pub fn parse_iso8601(timestamp: &str) -> PluginResult<DateTime<Utc>> {
        DateTime::parse_from_rfc3339(timestamp)
            ...
            .map_err(|e| crate::error::PluginError::InvalidInput(format!("Invalid timestamp: {}", e)))
    }
...
pub mod json {
    ...
    pub fn get_nested(value: &Value, path: &str) -> Option<&Value> {
        path.split('.')
    ...
    pub fn set_nested(value: &mut Value, path: &str, new_value: Value) -> PluginResult<()> {
        ...
        let last_key = keys.last().ok_or_else(|| {
            crate::error::PluginError::InvalidInput("Empty path".to_string())
        })?;
        
        let parent = keys[..keys.len() - 1].iter().fold(Ok(value), |current, key| {
            current.and_then(|v| {
                if !v.is_object() {
                    *v = serde_json::json!({});
                }
                v.as_object_mut()
                    .and_then(|obj| obj.entry(*key).or_insert_with(|| serde_json::json!({})).as_object_mut())
                    .map(|_| v)
                    .ok_or_else(|| crate::error::PluginError::InvalidInput("Invalid path".to_string()))
            })
        })?;
...
pub mod validation {
    ...
    pub fn validate_length(text: &str, min: usize, max: usize) -> PluginResult<()> {
        let len = text.len();
        if len < min {
            Err(crate::error::PluginError::InvalidInput(
    ...
    pub fn validate_required_fields(
        ...
        let obj = data.as_object().ok_or_else(|| {
            crate::error::PluginError::InvalidInput("Expected JSON object".to_string())
        })?;
        
        for field in required_fields {
            if !obj.contains_key(*field) {
                return Err(crate::error::PluginError::InvalidInput(
...
pub mod http {
    ...
    pub fn json_response<T: serde::Serialize>(status: u16, data: &T) -> PluginResult<HttpResponse> {
        let body = serde_json::to_string(data)
            .map_err(|e| crate::error::PluginError::SerializationError(e.to_string()))?;
...
pub mod logging {
    ...
    impl LogEntry {
        ...
        pub fn emit(self) {
            ...
            unsafe {
                crate::error::log_message(
                    self.level.as_str(),
                    &self.message,
                    &JsValue::from_serde(&metadata).unwrap_or(JsValue::NULL)
                );
            }
        }
    }
